...
import type { AdminDirectOffer } from "./types";

type DirectOfferCardProps = {
  offer: AdminDirectOffer;
};
...
function formatServices(value: string[] | string | null | undefined) {
  if (!value) return "Ei ilmoitettu";
...
export default function DirectOfferCard({
  offer,
}: DirectOfferCardProps) {
      ...
      {expanded && (
        <div className="border-t border-zinc-800 bg-zinc-950/70 p-6">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            <Detail label="Yritys" value={partner?.company} />
            <Detail label="Sähköposti" value={partner?.email} />
            <Detail label="Puhelin" value={partner?.phone} />
            <Detail label="Toiminta-alue" value={partner?.area} />
            <Detail label="Kategoria" value={partner?.category} />

            <Detail
              label="Palvelut"
              value={formatServices(partner?.services)}
            />
          </div>
